...
class TestClaudeCodeProviderInitialization:
    ...
    async def test_initialize_timeout(self, mock_tmux, mock_wait_status, mock_wait_shell, _):
        ...
        harness-control#225: this now raises TerminalInputBlockedError, not a bare TimeoutError --
        see initialize()'s own comment on that raise site. The message text is unchanged (still
        matched by gateway/init_race_retry.py's retry regex), only the exception TYPE changed, so
        this still needs updating: TerminalInputBlockedError is not a TimeoutError subclass.
        """
        mock_wait_shell.return_value = True
...
class TestClaudeCodeProviderStartupPrompts:
    ...
    @patch("cli_agent_orchestrator.backends.registry._backend")
    async def test_handle_startup_prompts_timeout(self, mock_tmux, mock_time, mock_sleep):
        ...
        provider = ClaudeCodeProvider("test123", "test-session", "window-0")
        await provider._handle_startup_prompts(timeout=20.0)
    ...
    def test_get_status_real_fullscreen_upsell_prompt_is_waiting_user_answer(self):
        """The real, live-captured "Try the new fullscreen renderer?" onboarding upsell (the
        concrete repro behind harness-control#225) must classify as WAITING_USER_ANSWER, not
        UNKNOWN -- the exact status initialize()'s wait_until_status call now also accepts as a
        real, alive, non-failure outcome. Nothing in this file answers the prompt -- see
        WAITING_USER_ANSWER_PATTERN's own comment for why auto-dismissal was explicitly rejected;
        this only lets CAO recognize the terminal is alive and blocked on something a human needs
        to see, not answer it on their behalf."""
        output = (
