...
class TestStaleProcessingCapturePane:
    ...
    def test_stale_processing_self_heals_via_capture_pane(self, mock_pm, mock_get_backend):
        ...
        provider.get_status.assert_called_once_with("the real pane -- idle composer, fully rendered")
    ...
    @patch("cli_agent_orchestrator.backends.registry.get_backend")
    @patch("cli_agent_orchestrator.services.status_monitor.provider_manager")
    def test_capture_pane_still_processing_stays_processing_no_crash(self, mock_pm, mock_get_backend):
        provider = MagicMock()
        provider.session_name = "s1"
        provider.window_name = "w1"
    ...
    @patch("cli_agent_orchestrator.backends.registry.get_backend")
    @patch("cli_agent_orchestrator.services.status_monitor.provider_manager")
    def test_no_provider_stays_processing_and_skips_capture_pane_entirely(self, mock_pm, mock_get_backend):
        mock_pm.get_provider.return_value = None
        backend = MagicMock()
        backend.supports_event_inbox.return_value = False
        mock_get_backend.return_value = backend
    ...
    @patch("cli_agent_orchestrator.backends.registry.get_backend")
    @patch("cli_agent_orchestrator.services.status_monitor.provider_manager")
    def test_capture_pane_fallback_retried_after_rate_limit_window(self, mock_pm, mock_get_backend, mock_time):
