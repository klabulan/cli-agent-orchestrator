...
# harness-control#225: broadened beyond the original arrow-key-navigate footer to also catch the
# "Enter to confirm · Esc to cancel" footer Ink's Select component renders for a plain
# numbered/lettered choice -- confirmed live via a real, deterministic repro
# (CLAUDE_CODE_FORCE_FULLSCREEN_UPSELL=1, the CLI's own env var for forcing its "Try the new
# fullscreen renderer?" first-run upsell, found via `strings` on the installed binary; see
# Tasks/20260716_hc225_onboarding_prompt_visibility/log.md) that this exact footer text is what a
# brand-new, never-before-seen CLI prompt used, distinct wording from the arrow-navigable case.
# This is deliberately generic CHROME text, not any one prompt's own wording -- the point is to
# classify a FUTURE, still-unrecognized choice-type prompt as WAITING_USER_ANSWER too, not just
# the specific prompts this file happens to already special-case by name below. Nothing here
# answers the prompt on the operator's behalf (operator's own explicit, binding instruction,
# harness-control#225: "don't press the question, let it reach the user") -- this only lets
# initialize() recognize a live, answerable prompt as a legitimate stopping point instead of an
# indistinguishable-from-broken timeout (see initialize()'s wait_until_status call below), so
# harness-control's own UI gets a chance to show it to a human instead of CAO tearing the session
# down first. TRUST_PROMPT_PATTERN/BYPASS_PROMPT_PATTERN are explicitly excluded below (get_status
# already did this before this change) so the trust/bypass dialogs -- which this file DOES
# actively dismiss, in _handle_startup_prompts -- are never reported as WAITING_USER_ANSWER while
# still unaccepted.
...
class ClaudeCodeProvider(BaseProvider):
    ...
    async def _handle_startup_prompts(self, timeout: Optional[float] = None) -> None:
    ...
    async def initialize(self) -> bool:
        ...
        #
        # harness-control#225: WAITING_USER_ANSWER added to this accept-set on purpose. Before
        # this change, ANY interactive choice-type prompt this file doesn't explicitly dismiss
        # (bypass/trust above) was structurally indistinguishable from a genuinely hung/broken
        # launch: both left the terminal sitting outside {IDLE, COMPLETED} until init_timeout, at
        # which point `create_terminal`'s own except-block tore the whole session down
        # (kill_session, FIFO stop, DB row deleted) -- so the operator never even got a CHANCE to
        # see and answer it. Confirmed live and 100% reproducible on an unpatched build via
        # CLAUDE_CODE_FORCE_FULLSCREEN_UPSELL=1 (see this repo's own
        ...
        # get_status()/get_status_from_screen()'s own WAITING_USER_ANSWER_PATTERN check, now
        # widened above to catch generic Ink confirm/select chrome, not just the arrow-navigable
        # case), so accepting it here cannot make initialize() return early on a blank/
        # still-launching terminal the way accepting UNKNOWN would.
        init_timeout = get_server_settings()["provider_init_timeout"]
        if not await wait_until_status(
            ...
            # harness-control#225: raise TerminalInputBlockedError, not a bare TimeoutError --
            # same message text as before (gateway/init_race_retry.py's own
            # _TRANSIENT_INIT_TIMEOUT_RE regex-matches this exact wording to decide whether a
            # SYNCHRONOUS create_session call is worth retrying; changing the wording would
            # silently break that unrelated mechanism, so it stays byte-identical). The type
            # change only matters on the deferred-init path (POST /sessions?defer_init=true):
